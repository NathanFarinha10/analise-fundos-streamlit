...
if not run_button:
    ...
    # --- ABA: Dashboard & Indicadores (GRANDES ALTERAÇÕES) ---
    with tab_dashboard:
        ...
        if not df.empty:
            ...
            df_fluxo_investidor = pd.DataFrame({
                'Investimento': -df['(+) Aportes'].clip(upper=0),
                'Distribuições': total_distribuido.clip(lower=0)
            })
